...
import { FieldTypeDraggable, DropZone } from './SimpleDragDrop';
...
interface SimpleFormBuilderWithDragDropProps {
    fields: FormField[];
    onFieldsChange: (fields: FormField[]) => void;
}

export const SimpleFormBuilderWithDragDrop: React.FC<SimpleFormBuilderWithDragDropProps> = ({
    fields,
    onFieldsChange
}) => {
    ...
    // Add field at specific index (for drag and drop)
    const addFieldAtIndex = useCallback((type: FieldType, index: number) => {
        const newField = createFieldFromType(type);
        const newFields = [...fields];
        newFields.splice(index, 0, newField);
        // Update order for all fields
        const reorderedFields = newFields.map((field, idx) => ({ ...field, order: idx }));
            ...
            <Card>
                ...
                <CardContent>
                    <ScrollArea className="h-[500px]">
                        {fields.length === 0 ? (
                            <DropZone
                                onDrop={addFieldAtIndex}
                                index={0}
                                className="relative min-h-[200px] border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center"
                            >
                            ...
                            <div className="space-y-2">
                                ...
                                {fields.map((field, index) => (
                                    <React.Fragment key={field.id}>
                                        <div
                                            className={`relative group border rounded-lg p-3 cursor-pointer transition-colors ${selectedFieldId === field.id
                                        ...
                                        >
                                            ...
                                            <div className="absolute left-1 top-1/2 transform -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <DragIndicator className="h-4 w-4 text-gray-400 cursor-grab" />
                                            </div>
                    ...
                    <ScrollArea className="h-[500px]">
                        {selectedField ? (
                            <div className="space-y-4">
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="field-description" className="text-sm">Description</Label>
                                    <Input
